	...
	float InStartTimeSeconds, FName InStartSection)
{
	...
	static bool bDisablePredictedProxyReactionVisualPath = true;
	if (bDisablePredictedProxyReactionVisualPath)
	{
		UE_LOG(LogTemp, Warning,
...
void USGM_MontageComponent::UpdateLocalProxyReactionMontage()
{
	...
	if (CurrentPosition >= LocalProxyReactionStartPosition && LocalProxyReactionMontage->HasRootMotion())
	{
		...
		NewMeshRelativeTransform.SetLocation(
			LocalProxyReactionOriginalMeshRelativeTransform.GetLocation() + CumulativeRelativeOffset);
